...
document.addEventListener('DOMContentLoaded', async function() {
  // Remove the existing scan button handler since we'll handle it in setupScanButton
  
  // Improved BGM handling from previous page
  ...
  if (shouldPlayBGM) {
    // Create new BGM with position from previous page
    bgm = new Audio('sound/bgm1.mp3');
    bgm.loop = true;
    bgm.volume = 0.5;
    
    // Set up play function that handles both user interaction and automatic resumption
    const playBGMFromSavedPosition = function() {
        bgm.currentTime = bgmPosition;
        bgm.play()
            .then(() => {
                console.log("BGM resumed from position:", bgmPosition);
                // Update position periodically to maintain continuity
                setInterval(() => {
                    sessionStorage.setItem('bgmPosition', bgm.currentTime);
                }, 1000);
            })
    ...
    playBGMFromSavedPosition();
    
    // Also set up interaction-based playback as fallback
